...
public class PreExecuteTests
{
    ...
    public async Task ShouldGetPreExecutionActivitiesSequence_FromInitialActivity(TestService service)
    {
        ...
        var response = await service.Client.RpcPreExecution.WorkflowApiRpcPreExecuteAsync(new(processId, initialActivity.Name));
    ...
    public async Task ShouldGetPreExecutionActivitiesSequence_FromFinalActivity(TestService service)
    {
        ...
        var response = await service.Client.RpcPreExecution.WorkflowApiRpcPreExecuteAsync(new(processId, finalActivity.Name));
    ...
    public async Task ShouldExecute_WhenPermissionAllowed(TestService service)
    {
        ...
        var response = await service.Client.ExclusivePermissions(c => c.RpcPreExecution, [WorkflowApiOperationId.RpcPreExecute]).WorkflowApiRpcPreExecuteAsync(new(processId, initialActivity.Name));
    ...
    public async Task ShouldReturn403_WhenPermissionDenied(TestService service)
    {
        ...
        var exception = await Assert.ThrowsExceptionAsync<ApiException>(
            async () => await service.Client.ExclusivePermissions(c => c.RpcPreExecution, Array.Empty<string>()).WorkflowApiRpcPreExecuteAsync(new ())
        );
