...
public class GetProcessStatusTests
{
    ...
    public async Task ShouldExecute_WhenPermissionAllowed(TestService service)
    {
        ...
        var response = await service.Client.WithPermissions(c => c.RpcInstance, WorkflowApiOperationId.RpcGetProcessStatus)
    ...
    public async Task ShouldReturn403_WhenPermissionDenied(TestService service)
    {
        ...
        var exception = await Assert.ThrowsExceptionAsync<ApiException>(
            async () => await service.Client.WithPermissions(c => c.RpcInstance, Array.Empty<string>()).WorkflowApiRpcGetProcessStatusAsync(request)
        );
