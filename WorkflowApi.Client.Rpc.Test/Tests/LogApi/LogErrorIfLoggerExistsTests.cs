...
[TestClass]
public class LogErrorIfLoggerExistsTests
{
    [ClientTest(HostId.RpcHost)]
    [TestMethod]
    public async Task ShouldLogErrorIfLoggerExistsMessageWithParameters(TestService service)
    {
        // Arrange
        ...
        await service.Client.RpcLog.WorkflowApiRpcLogErrorIfLoggerExistsAsync(new(message, logParameters));
    ...
    [ClientTest(HostId.RpcHost)]
    [TestMethod]
    public async Task ShouldLogErrorIfLoggerExistsMessage(TestService service)
    {
        // Arrange
        ...
        await service.Client.RpcLog.WorkflowApiRpcLogErrorIfLoggerExistsAsync(new(message));
    ...
    public async Task ShouldExecute_WhenPermissionAllowed(TestService service)
    {
        ...
        await service.Client.ExclusivePermissions(c => c.RpcLog, OperationId.RpcLogErrorIfLoggerExists).WorkflowApiRpcLogErrorIfLoggerExistsAsync(new("Test message"));
    }
