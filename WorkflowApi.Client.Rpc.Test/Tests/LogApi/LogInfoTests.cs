...
public class LogInfoTests
{
    ...
    [TestMethod]
    public async Task ShouldLogInfoMessageWithParameters(TestService service)
    {
        // Arrange
        ...
        await service.Client.RpcLog.WorkflowApiRpcLogInfoAsync(new("Test message", logParameters));
    ...
    [ClientTest(HostId.RpcHost)]
    [TestMethod]
    public async Task ShouldLogInfoMessage(TestService service)
    {
        //Arrange
        ...
        await service.Client.RpcLog.WorkflowApiRpcLogInfoAsync(new("Test message"));
    ...
    public async Task ShouldExecute_WhenPermissionAllowed(TestService service)
    {
        ...
        await service.Client.ExclusivePermissions(c => c.RpcLog, WorkflowApiOperationId.RpcLogInfo).WorkflowApiRpcLogInfoAsync(new("Test message"));
    }
    ...
    public async Task ShouldReturn403_WhenPermissionDenied(TestService service)
    {
        ...
        var exception = await Assert.ThrowsExceptionAsync<ApiException>(async () => await service.Client.ExclusivePermissions(c => c.RpcLog, Array.Empty<string>()).WorkflowApiRpcLogInfoAsync(new LogInfoRequest("Test message")));
