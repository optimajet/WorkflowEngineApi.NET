...
public class LogDebugIfLoggerExistsTests
{
    ...
    public async Task ShouldExecute_WhenPermissionAllowed(TestService service)
    {
        ...
        await service.Client.WithPermissions(c => c.RpcLog, WorkflowApiOperationId.RpcLogDebugIfLoggerExists).WorkflowApiRpcLogDebugIfLoggerExistsAsync(new("Test message"));
    }
    ...
    public async Task ShouldReturn403_WhenPermissionDenied(TestService service)
    {
        ...
        var exception = await Assert.ThrowsExceptionAsync<ApiException>(async () => await service.Client.WithPermissions(c => c.RpcLog, Array.Empty<string>()).WorkflowApiRpcLogDebugIfLoggerExistsAsync(new ("Test message")));
