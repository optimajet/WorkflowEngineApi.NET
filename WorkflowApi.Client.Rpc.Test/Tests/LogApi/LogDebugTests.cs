...
public class LogDebugTests
{
    ...
    [TestMethod]
    public async Task ShouldLogDebugMessageWithParameters(TestService service)
    {
        // Arrange
        ...
        await service.Client.RpcLog.WorkflowApiRpcLogDebugAsync(new(message, logParameters));
    ...
    [ClientTest(HostId.RpcHost)]
    [TestMethod]
    public async Task ShouldLogDebugMessage(TestService service)
    {
        //Arrange
        ...
        await service.Client.RpcLog.WorkflowApiRpcLogDebugAsync(new(message));
    ...
    public async Task ShouldThrowErrorWhenLoggerNotExist(TestService service)
    {
        ...
        var exception = await Assert.ThrowsExceptionAsync<ApiException>(async () =>
            await service.Client.RpcLog.WorkflowApiRpcLogDebugAsync(new("Test message"))
        );
    ...
    public async Task ShouldExecute_WhenPermissionAllowed(TestService service)
    {
        ...
        await service.Client.WithPermissions(c => c.RpcLog, WorkflowApiOperationId.RpcLogDebug).WorkflowApiRpcLogDebugAsync(new("Test message"));
    }
    ...
    public async Task ShouldReturn403_WhenPermissionDenied(TestService service)
    {
        ...
        var exception = await Assert.ThrowsExceptionAsync<ApiException>(async () => await service.Client.WithPermissions(c => c.RpcLog, Array.Empty<string>()).WorkflowApiRpcLogDebugAsync(new LogDebugRequest("Test message")));
