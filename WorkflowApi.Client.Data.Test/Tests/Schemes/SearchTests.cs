...
public class SearchTests
{
    ...
    public async Task ExecuteWithSchemeEqualTest(TestService service)
    {
        ...
        foreach (var model in result.Collection)
        {
            var expectedSchemaString = Converter.ToSchemaString(expected);
            var actualSchemaString = Converter.ToSchemaString(model.Scheme);
    ...
    [ClientTest(HostId.DataHost, ExcludeProviders = [ProviderName.Mongo])] // Mongo does not support collection equality
    [TestMethod]
